...
function Profile() {
  ...
  const _handleAvatarChange = async (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      setApiError('Vui lòng chọn file ảnh');
      return;
    }
    ...
    try {
      ...
      if (!token) {
        setApiError('Vui lòng đăng nhập lại');
      ...
      console.log('Uploading avatar to:', `${API_BASE_URL}/users/me/avatar`);
      ...
      setApiError(error.message || 'Có lỗi xảy ra khi upload ảnh');
    } finally {
      setUploadingAvatar(false);
    }
  };
  ...
  const _handleRemoveAvatar = async () => {
    ...
    try {
      ...
      setApiError(error.message || 'Có lỗi xảy ra khi xóa ảnh');
    } finally {
      setUploadingAvatar(false);
    }
  };
