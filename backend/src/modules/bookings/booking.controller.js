...
class BookingController {
  ...
  /**
   * Check homestay availability for dates
   * GET /api/v1/bookings/check-availability/:homestayId
   */
  checkAvailability = catchAsync(async (req, res) => {
    ...
    if (!checkInDate || !checkOutDate) {
      return ApiResponse.error(res, 'Check-in and check-out dates are required', 400);
    }

    const result = await bookingService.checkBookingAvailability(
      homestayId,
      checkInDate,
      checkOutDate
    );
    ...
    ApiResponse.success(
      res,
      result,
      result.available ? 'Homestay is available' : 'Homestay is not available for selected dates'
    );
  });

  /**
   * Get booked dates for a homestay
   * GET /api/v1/bookings/booked-dates/:homestayId
   */
  getBookedDates = catchAsync(async (req, res) => {
    const { homestayId } = req.params;
    const bookedDates = await bookingService.getBookedDates(homestayId);
    
    ApiResponse.success(res, { bookedDates }, 'Booked dates retrieved successfully');
  });
}
