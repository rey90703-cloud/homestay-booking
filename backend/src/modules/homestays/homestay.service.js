...
const { createCityRegex, getCityVariations } = require('../../utils/location.util');
...
class HomestayService {
  ...
  async createHomestay(hostId, homestayData) {
    ...
    // Geocode address if not already geocoded (double-check from controller)
    if (dataToSave.location && dataToSave.location.address && dataToSave.location.city) {
      if (!dataToSave.location.coordinates || 
          (dataToSave.location.coordinates.coordinates[0] === 105.8342 && 
           dataToSave.location.coordinates.coordinates[1] === 21.0278)) {
        // Coordinates are default or missing, try geocoding again
        try {
          const fullAddress = `${dataToSave.location.address}, ${dataToSave.location.city}, ${dataToSave.location.country || 'Vietnam'}`;
          logger.info(`🗺️ Service: Re-geocoding address: ${fullAddress}`);
          
          const geocodeResult = await geocodingService.geocodeAddress(fullAddress, dataToSave.location.city);
          
          if (geocodeResult && geocodeResult.lat && geocodeResult.lng) {
            dataToSave.location.coordinates = {
              type: 'Point',
  ...
  async updateHomestay(homestayId, hostId, userRole, updateData) {
    ...
    if (dataToUpdate.location) {
      ...
      // Geocode if address or city changed and coordinates are default or missing
      const addressChanged = dataToUpdate.location.address || dataToUpdate.location.city;
      const hasDefaultCoords = homestay.location.coordinates && 
                               homestay.location.coordinates.coordinates[0] === 105.8342 && 
                               homestay.location.coordinates.coordinates[1] === 21.0278;
      
      if (addressChanged && (hasDefaultCoords || !homestay.location.coordinates)) {
        try {
          const fullAddress = `${homestay.location.address}, ${homestay.location.city}, ${homestay.location.country || 'Vietnam'}`;
          logger.info(`🗺️ Service: Re-geocoding updated address: ${fullAddress}`);
          
          const geocodeResult = await geocodingService.geocodeAddress(fullAddress, homestay.location.city);
          
          if (geocodeResult && geocodeResult.lat && geocodeResult.lng) {
            homestay.location.coordinates = {
              type: 'Point',
              coordinates: [geocodeResult.lng, geocodeResult.lat],
            };
            if (geocodeResult.placeId) {
  ...
  async searchHomestays(searchParams) {
    ...
    // Location search with city variations support
    if (city) {
      const cityRegex = createCityRegex(city);
      query['location.city'] = cityRegex;
    }
    if (country) {
      query['location.country'] = createSafeRegex(country);
    }
    if (location && !search) {
      const cityRegex = createCityRegex(location);
      query.$or = [
    ...
    // Amenities - filter by slug (support both underscore and hyphen)
    if (amenities) {
      const amenityArray = Array.isArray(amenities) ? amenities : amenities.split(',');
      // Normalize slugs: convert underscore to hyphen
