...
class HomestayController {
  ...
  createHomestay = catchAsync(async (req, res) => {
    ...
    // Handle location - geocode address to get coordinates
    if (homestayData.location && homestayData.location.address && homestayData.location.city) {
      try {
        const fullAddress = `${homestayData.location.address}, ${homestayData.location.city}, ${homestayData.location.country || 'Vietnam'}`;
        logger.info(`🗺️ Geocoding address: ${fullAddress}`);
        
        const geocodeResult = await geocodingService.geocodeAddress(fullAddress, homestayData.location.city);
        
        if (geocodeResult && geocodeResult.lat && geocodeResult.lng) {
          homestayData.location.coordinates = {
            type: 'Point',
            coordinates: [geocodeResult.lng, geocodeResult.lat], // [longitude, latitude]
          };
    ...
    } else if (!homestayData.location || !homestayData.location.coordinates) {
      // Ensure coordinates exist with default
  ...
  updateHomestay = catchAsync(async (req, res) => {
    ...
    if (homestayData.location && (homestayData.location.address || homestayData.location.city)) {
      try {
        // Get existing homestay to merge with new data
        const existingHomestay = await homestayService.getHomestayById(req.params.id, req.user._id);
        const mergedLocation = {
          ...existingHomestay.location,
          ...homestayData.location,
        };
        
        const fullAddress = `${mergedLocation.address}, ${mergedLocation.city}, ${mergedLocation.country || 'Vietnam'}`;
        logger.info(`🗺️ Geocoding updated address: ${fullAddress}`);
        
        const geocodeResult = await geocodingService.geocodeAddress(fullAddress, mergedLocation.city);
