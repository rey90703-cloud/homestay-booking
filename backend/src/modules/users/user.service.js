...
class UserService {
  ...
  async updateAvatar(userId, fileBuffer) {
    ...
    try {
      ...
      if (isCloudinaryConfigured) {
        ...
        logger.warn('Cloudinary not configured, saving avatar as base64');
        const base64 = fileBuffer.toString('base64');
        const mimeType = 'image/jpeg'; // Default, should detect from buffer
  ...
  async deleteUser(userId) {
    ...
    logger.info(`User permanently deleted by admin: ${userId}`);
