...
const loadConfig = () => {
  const config = {
    ...
    bank: {
      ...
      expectedAccount: process.env.BANK_ACCOUNT_EXPECTED || process.env.BANK_ACCOUNT_NUMBER,
    },
...
const validateConfig = (config) => {
  ...
  if (!config.bank.bankName) {
    errors.push('BANK_NAME is required');
  }
  if (!config.bank.bankBin) {
    errors.push('BANK_BIN is required');
  }
